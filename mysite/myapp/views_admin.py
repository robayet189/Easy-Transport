...
def admin_fleet(request):
    ...
    return render(request, 'app1/admin/admin_fleet.html', {
        'active': 'fleet', 
        'buses': buses, 
...
def admin_schedule(request):
    ...
    today = timezone.now().date()
    return render(request, 'app1/admin/admin_schedule.html', {
        'active': 'schedule', 
        'schedules': schedules, 
        'routes': routes, 
        'drivers': drivers, 
...
def admin_revenue(request):
    today = timezone.now().date()
    return render(request, 'app1/admin/admin_revenue.html', {
        'active': 'revenue', 
        'today_revenue': Booking.objects.filter(schedule__travel_date=today, status='approved').aggregate(total=Sum('amount'))['total'] or 0, 
        'total_revenue': Booking.objects.filter(status='approved').aggregate(total=Sum('amount'))['total'] or 0
    })
...
def admin_alerts(request):
    return render(request, 'app1/admin/admin_alerts.html', {
        'active': 'alerts', 
...
# ==================== ✅ FIXED: admin_add_schedule - 100% Working ====================
@login_required
@user_passes_test(is_admin)
def admin_add_schedule(request):
    ...
    try:
        ...
        if not all([route_id, bus_id, travel_date, departure_time_str]):
            return JsonResponse({'success': False, 'message': 'Route, Bus, Date, Time are required'}, status=400)

        route = get_object_or_404(Route, id=route_id)
        bus = get_object_or_404(Bus, id=bus_id)

        from datetime import datetime
        departure = datetime.strptime(departure_time_str, '%H:%M').time()
        arrival = datetime.strptime(arrival_time_str, '%H:%M').time() if arrival_time_str else None

        if Schedule.objects.filter(route=route, travel_date=travel_date, departure_time=departure).exists():
            return JsonResponse({'success': False, 'message': 'Schedule already exists for this time'}, status=400)

       
        driver_obj = None
        if driver_id and str(driver_id).strip().lower() not in ['null', 'undefined', 'none', '']:
            try:
                driver_obj = Driver.objects.get(id=int(driver_id), is_approved=True, is_active=True)
            except (Driver.DoesNotExist, ValueError):
                driver_obj = None

     
        schedule = Schedule.objects.create(
            route=route, bus=bus, driver=driver_obj,
            departure_time=departure, arrival_time=arrival,
            travel_date=travel_date, fare=fare,
        ...
        if driver_obj:
            try:
                Trip.objects.create(
                    schedule=schedule, driver=driver_obj, route=route, bus=bus,
                    travel_date=travel_date, departure_time=departure, arrival_time=arrival,
                    status='pending'
                )
                driver_obj.assigned_bus = bus
                driver_obj.assigned_route = route
                driver_obj.save()
                trip_created = True
                
              
                Notification.objects.create(
                    type='driver', title='New Trip Assigned',
                    message=f'Driver {driver_obj.user.get_full_name()} assigned to Route {route.code}',
                    related_driver=driver_obj, is_read=False
                )
            except Exception as e:
                print(f"⚠️ Trip creation warning: {e}")

        msg = 'Schedule created successfully!'
        if trip_created:
            msg += ' Trip assigned to driver dashboard.'
        elif driver_id and driver_obj is None:
            msg += ' Driver not found or not approved.'
        else:
            msg += ' No driver assigned.'
...
def admin_get_bus(request, bus_id):
    if request.method == 'GET':
        bus = get_object_or_404(Bus, id=bus_id)
        return JsonResponse({'success': True, 'bus': {
            'id': bus.id, 
...
def admin_get_schedule(request, schedule_id):
    if request.method == 'GET':
        ...
        return JsonResponse({'success': True, 'schedule': {
            ...
            'route': s.route.id, 
            'bus': s.bus.id, 
            'driver': s.driver.id if s.driver else None, 
...
def admin_update_schedule(request, schedule_id):
    if request.method in ['POST', 'PUT']:
        s = get_object_or_404(Schedule, id=schedule_id)
        data = json.loads(request.body)
        s.route = get_object_or_404(Route, id=data.get('route', s.route.id))
        s.bus = get_object_or_404(Bus, id=data.get('bus', s.bus.id))
        
        # Handle driver assignment
        if data.get('driver'):
            try:
                s.driver = get_object_or_404(Driver, id=int(data.get('driver')), is_approved=True, is_active=True)
            except:
                s.driver = None
        else:
            s.driver = None
        
        s.travel_date = datetime.strptime(data.get('travel_date'), '%Y-%m-%d').date()
        s.departure_time = datetime.strptime(data.get('departure_time'), '%H:%M').time()
...
def admin_assign_driver_to_schedule(request, schedule_id):
    """Assign driver to an existing schedule and create Trip"""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Invalid method'}, status=400)
    ...
    try:
        ...
        driver = get_object_or_404(Driver, id=driver_id, is_approved=True, is_active=True)
        
        # Update schedule with driver
...
def admin_emergency_tab(request):
    ...
    notifications = Notification.objects.filter(type='emergency').order_by('-created_at')[:20]
